...
import 'package:to_do_app/core/constants/global_constants.dart';
...
class EditTasks extends ConsumerStatefulWidget {
  ...
  const EditTasks({Key? key, required this.data}) : super(key: key);

  @override
  ConsumerState<EditTasks> createState() => _EditUserState();
}

class _EditUserState extends ConsumerState<EditTasks> {
            ...
            InkWell(
              onTap: (){
                if(title.text.trim()!=''&&description.text.trim()!=''&&status!=null){
                  ...
                  final update=widget.data.copyWith(
                      title:title.text.trim(),
                      status:status,
                      description:description.text.trim(),
                  );

                  updateTasks(context,update);
  ...
  updateTasks(BuildContext context,TasksModel tasksModel) {

    ref.read(tasksControllerProvider.notifier).updateTasks(context: context, tasksModel: tasksModel);
